...
// and an inverted foreground/background pair clamps to black-on-white.
...
const cases = [
  ...
  ["inverted", "#e7efe9", "#15211b", true], // inverted → must clamp to b/w
...
check(afmco.dark !== "#000000", "the brand foreground should not clamp to black");

if (failed) {
  console.error(`${failed} qrColor invariant check(s) failed`);
  process.exit(1);
}
console.log(`qrColor invariants: ${cases.length} color pairs ok (contrast >= ${MIN_RATIO}, foreground<background, inverted→b/w)`);
