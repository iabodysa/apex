...
function requestTitle(request) {
  ...
  if (request.service_line === "Administrative Trip") {
    return to ? __("Administrative trip to {0}", [to]) : __("Administrative trip for {0}", [request.representative_label || request.requester_label || request.requester_name || __("representative")]);
  }
  return to || from || request.request_type || request.service_line || __("Transport Request");
}
...
<template>
  <SupervisorCollection
    :title="__('Transport Requests')"
    :description="__('Worker requests ordered to plan the next trip and track its status.')"
    icon="lucide-inbox"
    :resource="requests"
    date-field="pickup_datetime"
    :status-options="requestStatusOptions"
    :empty="__('No transport requests need follow-up.')"
  >
        ...
        <li v-for="(request, index) in rows" :key="request.name" class="supervisor-request-card">
          ...
          <RouterLink class="supervisor-request-card__copy" :to="`/requests/${encodeURIComponent(request.name)}`">
            ...
            <div class="supervisor-route-line">
              <span dir="auto">{{ request.from_location || request.accommodation_building_label || __("Origin location not specified") }}</span>
              <span aria-hidden="true">←</span>
              <span dir="auto">{{ request.to_location || __("Destination not specified") }}</span>
            </div>
            <div class="supervisor-meta-line">
              ...
              <span>{{ dateTimeLabel(request.pickup_datetime) || __("Time to be set later") }}</span>
              <span v-if="request.worker_count"><bdi>{{ request.worker_count }}</bdi> {{ __("Worker") }}</span>
              ...
              <span v-if="request.assigned_to_trip" class="record-identity" dir="auto">
                {{ __("Within") }} {{ request.assigned_trip_label || __("assigned trip") }}
