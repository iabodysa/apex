<script setup>
import { Button, FormControl, createListResource } from "frappe-ui";
import { createDraftAction } from "../worker/asyncState.js";

const plans = createListResource({ doctype: "Route Plan", auto: false });
...
function save() {
  return action.submit((values) => plans.insert.submit(values));
}
</script>
  ...
  <section class="feature-page">
    <header class="feature-page__heading">
      <div>
        ...
        <h2>خطة مسار جديدة</h2>
        <p>أنشئ الخطة ثم أكمل نقاط التوقف من مستندها.</p>
      </div>
    </header>
    <form class="feature-form" @submit.prevent="save">
      ...
      <p v-if="action.state.value === 'error'" class="feature-error" role="alert">
        {{ action.error.value }}
      </p>
