...
async function ratify() {
  ...
  try {
    ...
    toast.create({ type: "success", message: __("The round was approved") });
    await Promise.all([round.reload(), executions.reload()]);
  } catch (exception) {
    error.value = safeErrorMessage(exception, __("Could not approve the round."));
  }
}
</script>
<template>
    ...
    <template v-else>
      <div class="feature-card">
        <div class="record-identity">
          <strong dir="auto">{{ round.doc?.building || __("Safety Round") }}</strong>
      ...
      <ul class="feature-page__list">
        <li v-for="row in rows" :key="row.name">
          <div class="record-identity">
            <strong dir="auto">{{ row.task_title || __("Safety Item") }}</strong>
          ...
          <a v-if="row.evidence_photo" :href="row.evidence_photo" target="_blank" rel="noopener">
            <img class="safety-evidence" :src="row.evidence_photo" :alt="__('Observation Photo')" />
          </a>
          <RouterLink v-if="row.linked_maintenance_request" :to="`/maintenance/${row.linked_maintenance_request}`">{{ __("Maintenance Request") }} <bdi dir="auto" translate="no">{{ row.linked_maintenance_request }}</bdi></RouterLink>
        </li>
      </ul>
      <ErrorMessage v-if="error" :message="error" />
      <Button v-if="canReview && round.doc?.docstatus === 0" theme="green" variant="solid" :loading="submit.loading" @click="ratify">{{ __("Approve Round") }}</Button>
      <p v-else-if="round.doc?.docstatus === 0" class="feature-page__empty">{{ __("The round is awaiting review by the housing supervisor.") }}</p>
    </template>
  </section>
</template>
