...
const form = reactive({ room: "", issue_type: "Other", issue_description: "", priority: "Medium" });
const error = ref("");
const insert = createResource({ url: "frappe.client.insert" });
const rooms = createResource({ url: "frappe.client.get_list" });
const roomOptions = computed(() => (rooms.data || []).map((row) => ({
...
<template>
  <section class="feature-page"><h2>طلب صيانة جديد</h2><BuildingPicker />
    <form class="feature-form" @submit.prevent="submit">
      ...
      <FormControl v-model="form.issue_type" type="select" label="نوع المشكلة" :options="[
      ...
      <FormControl v-model="form.priority" type="select" label="الأولوية" :options="[
      ...
      <Button type="submit" variant="solid" :loading="insert.loading" :disabled="!building || !form.room || !form.issue_description">إرسال الطلب</Button>
    </form>
  </section>
</template>
