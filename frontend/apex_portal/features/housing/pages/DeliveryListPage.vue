...
<template>
  <ResourceListPage :title="__('Asset Handover')" :rows="deliveries.data || []" :loading="deliveries.list.loading" :error="deliveries.list.error" :refresh="deliveries.fetch">
    <template #row="{ row }">
      <RouterLink :to="`/delivery/${row.name}`">
        <div class="record-identity">
          <strong dir="auto">{{ row.asset_name || __("Housing Asset") }}</strong>
