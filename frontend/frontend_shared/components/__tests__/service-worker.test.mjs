...
// hand-maintained files had — most importantly that masar caches self-hosted fonts and
// driver does NOT (and that neither worker's network-first shell/data semantics drift,
// which is what caused the driver PWA spurious-refresh bug).
...
describe("portal service worker — activate cleanup", () => {
  it("deletes stale caches from prior versions but keeps the current shell/data caches", async () => {
    ...
    expect(sw.cacheStorage.has("driver-pwa-v0-oldhash-shell")).toBe(true);
...
describe("portal service worker — data fetch cache-hit", () => {
  it("serves the data API from cache when offline after a prior online read (network-first + fallback)", async () => {
    const sw = loadSW(DRIVER);
    const ep = DRIVER.dataEndpoints[0];
...
describe("portal service worker — update via SKIP_WAITING", () => {
  it("activates the waiting worker only on a SKIP_WAITING message", async () => {
    const sw = loadSW(DRIVER);
