...
const ALLOWED = new Map([
  ...
  [
    PORTALS.map((p) => `${p}:auto-imports.d.ts`).join(" | "),
    "Generated by the frappe-ui Vite plugin (it bundles unplugin-auto-import) on " +
      "every build, and empty because no portal configures auto-imports — so it is " +
      "identical across all seven by construction. Deleting it only makes the next " +
      "build write it back byte-identically; it is editor typing support, nothing " +
      "imports it. Untracking it needs a repo-root .gitignore rule.",
  ],
