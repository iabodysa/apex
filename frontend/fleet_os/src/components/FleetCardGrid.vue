        ...
        <div v-else class="cards-grid" :class="{ 'fp-compact': density === 'compact' }">
          ...
          >
            ...
            <div class="vc-actions" :class="{ 'fp-actions-busy': isBusy(v.plate) }" @click.stop>
              ...
              <button v-if="v.vehicle_status === 'stolen'" class="ac" style="border-color:color-mix(in srgb,var(--critical) 25%,transparent);color:var(--critical-l)" :disabled="isBusy(v.plate)" :title="t('card.recoverTitle')" @click="recoverVehicle(v.plate)"><span class="ac-ico"><Icon name="lock-open" :size="14" /></span>{{ t("card.recover") }}</button>
