...
<template>
  ...
  <template v-else>
    <div class="section-head">
      ...
      <Badge theme="green" size="lg" :label="t('arrivals.arrived', { arrived: manifest.arrived, total: manifest.total })" />
      <Button variant="ghost" size="md" :label="t('common.refresh')" @click="expectedRes.reload()">
        <template #icon><Icon name="refresh" :size="18" /></template>
      </Button>
    </div>

    <ListSkeleton v-if="expectedRes.loading && !manifest.total" :rows="4" :label="t('common.loading')" />
    ...
    <ul v-else class="stack-tight">
      <li v-for="row in manifest.workers" :key="row.row" class="row-card">
        <div class="row-body">
          <b>{{ row.worker_name }}</b>
          <small>{{ row.passport_number }} · {{ row.nationality || t("common.none") }}</small>
        </div>
        <span v-if="row.arrived" class="pill pill-success">{{ t("arrivals.registered") }}</span>
        <Button
          v-else
          size="lg"
          variant="subtle"
    ...
    <ul v-if="workers.length" class="stack-tight">
      <li v-for="w in workers" :key="w.party_type + w.party" class="row-card">
        ...
        <Button size="lg" variant="subtle" :label="t('arrivals.houseCta')" @click="houseWorker(w)" />
        <Button size="lg" variant="ghost" :label="t('arrivals.slipCta')" @click="showSlip(w)" />
        <Button
    ...
    <Dialog
      ...
      :options="{ title: t('arrivals.registerTitle'), size: 'lg' }"
    >
      <template #body-content>
        <div class="stack">
          <FormControl type="text" size="lg" :label="t('arrivals.workerName')" v-model="form.worker_name" />
          <FormControl type="text" size="lg" :label="t('arrivals.passport')" v-model="form.passport_number" />
          <FormControl
            type="select"
            size="lg"
          ...
          />
          <FormControl type="text" size="lg" :label="t('arrivals.phone')" v-model="form.cell_number" />
          <FormControl type="text" size="lg" :label="t('arrivals.iqama')" v-model="form.iqama_number" />
    ...
    <Dialog v-model="linkOpen" :options="{ title: t('arrivals.linkCta'), size: 'sm' }">
      ...
      <template #actions>
        <Button size="2xl" variant="solid" :label="t('common.close')" @click="linkOpen = false" />
      </template>
    </Dialog>
    ...
    <Dialog v-model="slipOpen" :options="{ title: t('arrivals.slipTitle'), size: 'xl' }">
      ...
      <template #actions>
        <div class="sheet-actions">
          <Button size="2xl" variant="solid" :label="t('arrivals.slipPrint')" @click="printSlip" />
          <Button size="xl" variant="outline" :label="t('common.close')" @click="slipOpen = false" />
        </div>
      </template>
    </Dialog>
  </template>
...
async function doRegister() {
  ...
  try {
    ...
    toast.create({ type: "success", message: t("arrivals.registered_ok", { name: created.label }) });
