...
- **Operator desk pages** — single-screen Frappe desk pages for high-volume work: the Front Desk bed board and the Arrivals Desk, a building-first check-in screen (live floor-map, worker search, passport register, one-click housing with over-capacity, custody handover, arrival card and multi-passenger transport) that orchestrates the existing party-aware writers and adds no posting logic of its own.
