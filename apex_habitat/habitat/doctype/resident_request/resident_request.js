	...
	refresh(frm) {
		...
		if (can_convert) {
			frm.add_custom_button(__("Convert to Document"), function () {
				frappe.call({
					method: "apex_habitat.habitat.doctype.resident_request.resident_request.convert_request",
...
frappe.listview_settings["Resident Request"] = {
	...
	button: {
		...
		action(doc) {
			...
			frappe.call({
				method: "apex_habitat.habitat.doctype.resident_request.resident_request.advance_triage_status",
	...
	onload(listview) {
		listview.page.add_actions_menu_item(
			...
			() => {
				...
				frappe.call({
					method: "apex_habitat.habitat.doctype.resident_request.resident_request.bulk_triage",
					args: { names: names },
					freeze: true,
