<!-- Lightweight inline-SVG icon set (lucide geometry), so the portal needs no
     ...
     Directional icons (chevron) mirror under [dir="rtl"] via the CSS below. -->
<template>
  <svg
    ...
    aria-hidden="true"
    :class="['fp-icon', 'fp-icon-' + name]"
  >
    <template v-if="name === 'car'">
      <path d="M19 17h2c.6 0 1-.4 1-1v-3c0-.9-.7-1.7-1.5-1.9C18.7 10.6 16 10 16 10s-1.3-1.4-2.2-2.3c-.5-.4-1.1-.7-1.8-.7H5c-.6 0-1.1.4-1.4.9l-1.4 2.9A3.7 3.7 0 0 0 2 12v4c0 .6.4 1 1 1h2" />
      <circle cx="7" cy="17" r="2" />
      <path d="M9 17h6" />
      <circle cx="17" cy="17" r="2" />
    </template>
    <template v-else-if="name === 'bike'">
    ...
    <template v-else-if="name === 'triangle-alert'">
    ...
    <template v-else-if="name === 'circle-dot'">
      <circle cx="12" cy="12" r="10" />
      <circle cx="12" cy="12" r="1" />
    </template>
    <template v-else-if="name === 'circle-pause'">
      <circle cx="12" cy="12" r="10" />
    ...
    <template v-else-if="name === 'chart-column'">
...
<style scoped>
/* Inline icons sit on the text baseline by default; align to the surrounding
   line so they read as part of the label/button, not floating above it. */
.fp-icon {
