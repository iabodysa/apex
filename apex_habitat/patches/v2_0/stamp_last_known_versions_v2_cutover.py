...
[apex_habitat] to the FINAL v1 changelog version, so the v2.0.0 "Updated To A New
...
``User.last_known_versions["apex_habitat"]["version"]`` (default ``0.0.1``) and
LESS-OR-EQUAL to the running ``apex_habitat.__version__``. At the v2.0.0 cutover
...
def execute():
    ...
    for row in rows:
        ...
        if isinstance(entry, dict):
            ...
            known["apex_habitat"] = {"version": target_str}
