...
// socket. An enhancement over a manual reload — every failure path is swallowed
// so the portal's own fetch still carries it if the socket never connects.
import { io } from "socket.io-client";

const EVENT = "safety_update";
// A submitted/cancelled Safety Round changes which cadences are DUE, so the
// portal subscribes to that doctype's room and refetches the due set on a push.
const ROOM_DOCTYPE = "Safety Round";

// Socket config is injected by www/safety.py via a <script> in safety.html.
function cfg() {
...
// Start the realtime subscription. `onUpdate` runs on each received safety_update.
// Returns a teardown function; safe to call even if the socket never started.
export function connectSafetyRealtime(onUpdate) {
  ...
  try {
    ...
    return () => {}; // socket.io unavailable — the portal's fetch carries it
  }
  ...
  socket.on("connect_error", () => {
    /* swallow: the portal's own fetch is the fallback path */
  });
