...
(``project_scoped_has_permission``) — the two generic entry points every scoped
Salis DocType shares. Both read the DocType out of ``SALIS_SCOPE``: the list
fragment through ``doctype``, the document check through ``doc.doctype``, which
for Dispatch Trip selects the ``trip`` strategy with the ``trip_actor`` own-row
basis and the ``dispatch_trip`` document rule.
...
These tests lock down the second layer: the driver-own SQL fragment must select
the acting driver's OWN trips and exclude another driver's, and the document
check must allow a driver on their own trip and deny on another's. The
own-driver basis is resolved through the
...
class TestDispatchTripDriverScope(FrappeTestCase):
    ...
    def _trips_matching_query(self, user):
        ...
        cond = project_scope_query(user, doctype="Dispatch Trip")
