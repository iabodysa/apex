...
ALWAYS the owner at the create check, so the five handlers that tested ownership first
...
ownership is a durable fact and is sufficient again. The handlers stay deny-only and
...
result, and the allow and the refuse are asserted as a PAIR — a handler that lets every
create through would pass the allow half alone, and one that blocks every create would
...
TRIPS = {
    "TRIP-A": {"route_plan": "RP-A", "driver": "DRV-A"},
    "TRIP-B": {"route_plan": "RP-B", "driver": "DRV-B"},
}
...
# Each handler with the parent link that anchors it at the create check, in-scope and
...
CREATE_CASES = [
    (
        "Trip Start Log",
        SP.trip_start_log_has_permission,
    ...
    (
        "Salis Driver",
        SP.salis_driver_has_permission,
        {"project": PROJ_A},
        {"project": PROJ_B},
        {"project": None},
    ),
    (
        "Driver Attendance",
        SP.driver_attendance_has_permission,
        {"driver": "DRV-A"},
        {"driver": "DRV-B"},
        {"driver": None},
    ),
    ...
    (
        "Boarding Scan Log",
        SP.boarding_scan_log_has_permission,
...
DRIVER_SELF_CASES = [
    ("Trip Start Log", SP.trip_start_log_has_permission, "dispatch_trip", "TRIP-A", "TRIP-B"),
    ("Driver Attendance", SP.driver_attendance_has_permission, "driver", "DRV-A", "DRV-B"),
    ("Driver Suspension", SP.driver_stop_has_permission, "driver", "DRV-A", "DRV-B"),
    ("Boarding Scan Log", SP.boarding_scan_log_has_permission, "dispatch_trip", "TRIP-A", "TRIP-B"),
]
...
class TestAnUnsavedRowIsDecidedByItsParentNotItsOwner(_ScopeCase):
    ...
    def test_an_in_scope_parent_allows_the_create(self):
        self.scoped()
        for label, handler, in_scope, _out, _none in CREATE_CASES:
    ...
    def test_an_out_of_scope_parent_refuses_the_create(self):
        ...
        self.scoped()
        for label, handler, _in, out_of_scope, _none in CREATE_CASES:
            with self.subTest(doctype=label):
    ...
    def test_the_two_verdicts_are_never_the_same_verdict(self):
        ...
        self.scoped()
        for label, handler, in_scope, out_of_scope, _none in CREATE_CASES:
            with self.subTest(doctype=label):
                verdicts = (
                    handler(_doc(label, True, **in_scope), "create"),
                    handler(_doc(label, True, **out_of_scope), "create"),
                )
    ...
    def test_an_unanchored_create_fails_closed(self):
        ...
        self.scoped()
        for label, handler, _in, _out, unanchored in CREATE_CASES:
            with self.subTest(doctype=label):
                self.assertIs(handler(_doc(label, True, **unanchored), "create"), False)

    def test_a_scoped_user_holding_no_project_is_refused_too(self):
        self.scoped(projects=())
        for label, handler, in_scope, _out, _none in CREATE_CASES:
            with self.subTest(doctype=label):
                self.assertIs(handler(_doc(label, True, **in_scope), "create"), False)

    def test_oversight_still_defers(self):
        self.unscoped()
        for label, handler, _in, out_of_scope, _none in CREATE_CASES:
            with self.subTest(doctype=label):
                self.assertIsNone(handler(_doc(label, True, **out_of_scope), "create"))
...
class TestOwnershipStillDecidesAStoredRow(_ScopeCase):
    ...
    def test_the_owner_may_still_act_on_their_own_out_of_scope_row(self):
        ...
        for label, handler, _in, out_of_scope, _none in CREATE_CASES:
            with self.subTest(doctype=label):
                self.assertIsNone(
                    handler(_doc(label, False, **out_of_scope), "read"),
    ...
    def test_a_non_owner_is_still_allowed_an_in_scope_row(self):
        ...
        for label, handler, in_scope, _out, _none in CREATE_CASES:
            with self.subTest(doctype=label):
                ...
                self.assertIsNone(handler(doc, "read"))
    ...
    def test_a_doc_carrying_no_flag_is_treated_as_stored(self):
        ...
        self.assertIsNone(SP.driver_attendance_has_permission(doc, "create"))
...
class TestTheDriverSelfServiceCreateIsNotCollateral(_ScopeCase):
    ...
    def test_a_driver_may_create_their_own_row(self):
        ...
        for label, handler, field, mine, _theirs in DRIVER_SELF_CASES:
            with self.subTest(doctype=label):
                self.assertIsNone(
                    handler(_doc(label, True, **{field: mine}), "create"),
    ...
    def test_a_driver_may_not_create_on_another_drivers_parent(self):
        self.scoped(projects=())
        self.own_driver("DRV-A")
        for label, handler, field, _mine, theirs in DRIVER_SELF_CASES:
            with self.subTest(doctype=label):
                self.assertIs(
                    handler(_doc(label, True, **{field: theirs}), "create"),
                    False,
    ...
    def test_the_driver_basis_needs_a_resolved_driver(self):
        ...
        for label, handler, field, mine, _theirs in DRIVER_SELF_CASES:
            with self.subTest(doctype=label):
                self.assertIs(handler(_doc(label, True, **{field: mine}), "create"), False)


class TestTheHandlersStayPtypeAgnostic(_ScopeCase):
    """Proof 4 — the contract the obvious fix would have broken.

    These handlers are deny-only and ptype-agnostic by contract: the same denial applies
        ...
        for label, handler, in_scope, out_of_scope, _none in CREATE_CASES:
            for ptype in self.PTYPES:
                with self.subTest(doctype=label, ptype=ptype):
                    self.assertIsNone(handler(_doc(label, True, **in_scope), ptype))
                    self.assertIs(
                        handler(_doc(label, True, **out_of_scope), ptype), False
                    )

    def test_every_verdict_is_deny_only(self):
        """A handler may return False (block) or None (defer) — never a grant."""
        self.scoped()
        for label, handler, in_scope, out_of_scope, unanchored in CREATE_CASES:
            for fields in (in_scope, out_of_scope, unanchored):
                for unsaved in (True, False):
                    with self.subTest(doctype=label, unsaved=unsaved, fields=fields):
                        verdict = handler(_doc(label, unsaved, **fields), "create")
...
class TestTheGuardIsLoadBearing(_ScopeCase):
    ...
    reproduces the pre-A-233 handlers exactly — ownership tested first, unconditionally —
