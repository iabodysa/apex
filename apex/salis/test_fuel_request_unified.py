...
def _live_quota(vehicle, period_month, monthly_litres=100):
    """The live Fuel Quota for (vehicle, period_month), created when absent.

	``FuelQuota._guard_duplicate`` and a composite unique index allow only one row
	per vehicle and period below docstatus 2, and the vehicle outlives the test
	run, so this is get-or-create rather than an insert."""
...
class TestFuelRequestTopup(FrappeTestCase):
    """A Top-up raises the ceiling of a named allocation, so every request here
	carries a Fuel Quota — ``FuelRequest._validate_topup`` refuses one that does
	not. One live quota per (vehicle, period_month) is all the duplicate guard
	allows, so the class shares a single get-or-created row."""
    ...
    def setUpClass(cls):
        ...
        cls.quota = _live_quota(cls.vehicle, "2026-05")
