...
for the records that Employee owns: their active housing assignment, their
identity documents, their custody balance, their resident-request timeline, their
contacts, and the validation of what they may file.

Two kinds of function live here and the difference is deliberate. The readers
(``_active_assignment``, ``_custody_issued_by``, ``_building_in_charge``,
``_today_driver``, ``_hr_notify_recipients``) touch the database. The shapers
(``_iqama_of``, ``_worker_documents``, ``_net_custody_items``,
``_request_status_timeline``, ``_clean_adhoc_passengers``) take plain values and
return plain values, so each can be exercised without a bench or a site.
