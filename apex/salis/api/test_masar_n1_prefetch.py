...
class TestMasarWorkerRoutePrefetch(_WorkerTripMixin, FrappeTestCase):
    ...
    def _route_today_as(self, driver, user, workers, route_name):
        ...
        The trip is landed on ``Dispatched``: ``_today_worker_trips`` (masar_routes)
        selects Dispatch Trips at that status only, so a trip left on the insert-time
        ``Planned`` never reaches the read path and the payload comes back empty."""
        self._worker_trip(
            driver, self.project, self.building, workers, route_name, status="Dispatched"
        )
        frappe.set_user(user)
        try:
            # One un-counted call first. ``frappe.set_user`` drops the request-local
            # caches, so the first read after a user switch also loads DocType meta and
            # role permissions (Custom DocPerm / DocType Link / DocType Action /
            # DocType State). Those reads are per-DocType and depend on cache warmth,
            # not on how many workers the trip carries, so counting them would make the
            # comparison below measure whichever call ran first rather than the shape of
            # the read path.
