...
# The directory holding the INSTALLED apex/ package. Derived from this file's own path it
# named `.claude/tests`, so every source read below opened a file that is not there.
...
DRIVER_PORTAL_LIMITS = {
    ...
    ("execution.py", "mark_stop_progress"): 30,
    ("execution.py", "mark_arrived"): 30,
    ("personal.py", "get_masar_today"): 120,
    ("personal.py", "get_my_accommodation"): 60,
    ("personal.py", "get_my_custody"): 60,
    ("personal.py", "get_my_resident_requests"): 60,
    ("profile.py", "get_driver_profile"): 120,
    ("trips.py", "my_trips_today"): 120,
    ("trips.py", "my_trips_recent"): 120,
    ("trips.py", "my_worker_route_today"): 120,
    ("trips.py", "my_trip_route"): 120,
}
DRIVER_PORTAL_ENDPOINTS = set(DRIVER_PORTAL_LIMITS)

MIXED_DRIVER_LIMITS = {
    ("boarding_flow.py", "get_trip_boarding"): 120,
    ("boarding_flow.py", "notify_remaining_passengers"): 30,
    ("boarding_flow.py", "driver_mark_not_boarded"): 30,
    ("boarding_flow.py", "depart_and_finalize"): 30,
}
MIXED_DRIVER_ENDPOINTS = set(MIXED_DRIVER_LIMITS) | {
...
# Every driver endpoint that WRITES, and must therefore stay POST-only: a guest surface
# that mutates over GET is reachable from an <img> tag on any page the driver opens.
DRIVER_WRITERS = {
    ("execution.py", "start_my_trip"),
    ("execution.py", "complete_my_trip"),
    ("execution.py", "mark_stop_progress"),
    ("execution.py", "mark_arrived"),
    ("boarding.py", "scan_boarding_pass"),
...
# The one endpoint that CREATES the trip log the whole execution chain hangs off, held
# at the tightest ceiling in the portal. Named separately because "creation is tight" is
# the claim; the rest of the map is checked value-for-value.
TIGHT_DRIVER_WRITERS = {("execution.py", "start_my_trip")}
...
ENDPOINT_LIMITS = {**DRIVER_PORTAL_LIMITS, **MIXED_DRIVER_LIMITS}
...
class TestDriverPortalGuestInventory(unittest.TestCase):
    ...
    def test_driver_writer_limits_are_bounded_and_creation_is_tight(self):
        """Every metered endpoint's ceiling, value for value.

        Checked against the map rather than a tier lookup: a tiered assertion silently
        widens whenever a new tier appears (the 60-per-minute personal reads did exactly
        that), because the endpoint simply falls into the catch-all branch.
        """
        self.assertEqual(
            set(ENDPOINT_LIMITS),
            PER_IP_DRIVER_ENDPOINTS,
            "an endpoint gained or lost its decorator without this map moving",
        )
        for filename, name in sorted(PER_IP_DRIVER_ENDPOINTS):
            path = _endpoint_path(filename, name)
            node = _module_endpoints(path)[name][0]
            ...
            with self.subTest(path=filename, endpoint=name):
                self.assertIsNotNone(rate)
                self.assertEqual(
                    _literal_keywords(rate[1]).get("limit"), ENDPOINT_LIMITS[(filename, name)]
                )
        self.assertEqual(
            {ENDPOINT_LIMITS[endpoint] for endpoint in TIGHT_DRIVER_WRITERS},
            {10},
            "trip creation left the tightest ceiling in the portal",
        )
        self.assertLess(
            max(ENDPOINT_LIMITS[endpoint] for endpoint in DRIVER_WRITERS & set(ENDPOINT_LIMITS)),
            max(ENDPOINT_LIMITS.values()),
            "a writer is metered as loosely as the loosest read",
        )
