...
``test_ignore`` prunes the auto-dependency walk (Salis Driver -> Employee -> HR
masters, and Salis Vehicle).
"""
...
def _driver():
    name = frappe.db.get_value("Salis Driver", {"full_name": "TDR Realtime Driver"}, "name")
    if not name:
        name = (
            frappe.get_doc(
                {
                    "doctype": "Salis Driver",
                    "full_name": "TDR Realtime Driver",
                    "status": "Active",
                }
            )
