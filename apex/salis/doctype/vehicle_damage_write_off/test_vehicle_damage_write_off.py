...
def setUpModule():
    ...
    is evaluated at IMPORT time and turns exactly the seed regression this file exists to
    catch into a silent green run of its three cases. The sibling workflow suites in this
    tree (fuel_claim, fuel_exception_case) already made this call; this one was left
    behind. The class above it never guarded at all and would simply have gone red, which
    is the correct behaviour and is now the whole module's.
    """
    active = get_workflow_name("Vehicle Damage Write-Off")
    if active != WORKFLOW:
