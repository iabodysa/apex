...
from apex.salis.report.rental_cost_by_office.rental_cost_by_office import (
...
class TestRentalSettlementReconciliation(FrappeTestCase):
    ...
    def _purge_settlement(self, name):
        ...
        if doc.docstatus == 1:
            # Rental Settlement has a workflow cancel transition, so a bare cancel from a
            # non-cancel state (e.g. Paid) is refused by the A-083 guard; land the cancel
            # state first (guard fast-path) for teardown.
