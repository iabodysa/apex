...
class TestFuelRequest(FrappeTestCase):
	...
	def test_a_standard_request_with_no_linked_quota_is_not_graded(self):
		...
		doc._guard_quota_allowance(frappe._dict(consumed_litres=0, monthly_litres=1, status="Active"))
	...
	def test_topup_requires_positive_topup_litres(self):
		doc = self._request(request_type="Top-up", topup_litres=0, fuel_quota="_Test Quota")
	...
	def test_a_non_temporary_topup_needs_no_revert_due_date(self):
		doc = self._request(
			request_type="Top-up", topup_litres=5, fuel_quota="_Test Quota", is_temporary=0
		)
		doc._validate_topup()
