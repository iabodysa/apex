...
The row test seeds a driver and an open clearance: on an empty test database the
...
class TestDriverClearanceRegister(FrappeTestCase):
    ...
    def test_seeded_clearance_reports_its_driver_and_zero_outstanding_counters(self):
        _columns, data = execute({"driver": self.driver})
        ...
        # Server-derived on validate; a fresh driver has no open cases against them.
