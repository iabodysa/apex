...
Both classes work on the two shipped vehicle fixtures rather than minting their own. The
rental class needs both of them on rent, so it writes the ownership and office onto the
borrowed pair and hands both back at class teardown.
"""
...
class TestRentalAccrualLoopResilience(FrappeTestCase):
    ...
    def setUpClass(cls):
        ...
        cls.good_vehicle = cls._rented_vehicle(PLATE, "Owned")
        cls.bad_vehicle = cls._rented_vehicle(SECOND_PLATE, "Rented")

    @classmethod
    def _rented_vehicle(cls, plate, shipped_ownership):
        """Put a borrowed fixture vehicle on rent with an open Receipt, and register the
        restore of the ownership the fixture ships with."""
        name = frappe.db.get_value("Salis Vehicle", {"plate_number": plate}, "name")
        cls.addClassCleanup(
            frappe.db.set_value,
            "Salis Vehicle",
            name,
            {"ownership": shipped_ownership, "rental_office": None},
        )
        frappe.db.set_value(
            "Salis Vehicle", name, {"ownership": "Rented", "rental_office": cls.office}
        )
