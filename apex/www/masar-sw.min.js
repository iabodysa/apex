...
// covers /masar. Shell is NETWORK-FIRST; fonts are cache-first; stale owned caches
// drop on activate. BUILD (full asset-tree hash, stamped per build) changes these bytes on
// each deploy -> updatefound. Activates on SKIP_WAITING.
...
// self-hosted fonts. The HTML is cached under /masar (no query string) so
...
// The portal data API is issued by frappe-ui as a POST (args travel in the JSON
...
// Network-first for the portal data API (GET or POST): always try the network
