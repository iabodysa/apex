...
"""Guard: a www controller with no template serves a 404, not a page.
...
``www/housing_count.py`` has a sibling ``www/housing-count.html`` marker, which is what
lets the ``Inventory Count`` shortcut in the Housing workspace reach it instead of
404ing. Both baselines below are EMPTY and the guard runs at zero tolerance.
...
WWW = Path(apex.__file__).resolve().parent / "www"
APP_ROOT = WWW.parent
...
# SHRINK-ONLY, currently empty: an entry leaves when the route is fixed, and
# test_frozen_offenders_are_still_broken fails if one is fixed but not pruned. Adding
# an entry is a review decision, never a way to ship a new one.
_TEMPLATELESS_BASELINE: dict[str, str] = {}

# Shortcut targets no www template serves. Same shrink-only rule.
...
class TestWwwControllersHaveTemplates(unittest.TestCase):
    ...
    def test_housing_count_is_reachable_so_its_redirect_can_run(self):
        # Asserted along the resolver's own chain: a non-Python template exists,
        # set_pymodule maps it back to the controller, and the route appears in the
        # served set the shortcut check compares against.
