...
class _ScopeCase(unittest.TestCase):
    ...
    def _scope(self, unscoped, buildings):
        ...
        self._stub_user_permissions(buildings)
        return None

    def _stub_user_permissions(self, buildings):
        ...
        doctype. That read is real and is the point of the narrowing, so it is not
        removed here — it is answered with rows matching the scope this test already
        granted, each carrying no ``applicable_for``, which is the row shape a plain
        building grant produces. Values therefore pass the filter and the assertions
        below still grade the estate resolution rather than the narrowing.
        """
        rows = {"Building": [{"doc": b, "applicable_for": None} for b in buildings]}
        stub = patch.object(
...
class TestTheAnchorMapCoversTheWholeClass(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        for path in cls.APEX.rglob("*/doctype/*/*.json"):
            try:
                ...
                continue
            # The glob also reaches each DocType's test_records.json, which is a LIST.
    ...
    def test_every_wired_doctype_carries_the_fields_its_strategy_reads(self):
        ...
        The handler resolves an estate by one of four strategies declared in
        ``BUILDING_SCOPE``, so the field a DocType must carry is whichever one ITS
        strategy reads: one column for ``column``, both endpoints for ``dual``, the
        link for ``hop``, the child table for ``child``. Asking only for a field named
        ``building`` was right while every wired DocType was ``column``-kind and goes
        blind the moment one is not.
        """
        missing = []
        for doctype in self._wired_to_the_building_handler():
            if doctype == "Building":
            ...
            if not data:
                continue
            kind, spec = HP.BUILDING_SCOPE.get(doctype) or ("column", HP.BUILDING)
            if kind in HP.ANCHORED_KINDS and doctype in HP.BUILDING_FETCH_ANCHOR:
                # Scope resolves through the declared anchor instead of a local field.
                # test_each_anchor_names_a_link_that_actually_exists is what proves that
                # anchor real, so this is a redirect, not an escape.
                continue
            fieldnames = {f.get("fieldname") for f in data.get("fields", [])}
            if kind == "column":
                required = {spec["field"]}
            elif kind == "dual":
                required = {spec["first"], spec["second"]}
            elif kind == "hop":
                required = {spec["field"]}
            else:
                child = spec["child"]
                required = set()
                if not any(
                    f.get("fieldtype") == "Table" and f.get("options") == child
                    for f in data.get("fields", [])
                ):
                    required = {"a Table field holding " + child}
            absent = sorted(required - fieldnames)
            if absent:
                missing.append((doctype, kind, absent))
        self.assertEqual(
