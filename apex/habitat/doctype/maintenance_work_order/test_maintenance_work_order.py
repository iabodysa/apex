...
Size note: 508 test lines against 196 in the subject (2.6x), for 22 distinct behaviours. The ratio is arithmetic on a small subject, not overtesting: each test names a different behaviour.
"""
    ...
    def test_completion_posts_each_side_effect_exactly_once(self):
        ...
        with as_user(self.tech):
            mark_completed(
                wo.name,
                actual_end_date=today(),
                completion_photo="/files/a271-once.png",
                completion_notes="Fault cleared and retested on site",
            )
    ...
    def test_a_second_completion_attempt_posts_nothing(self):
        ...
        with as_user(self.tech):
            ...
            mark_completed(
                wo.name,
                actual_end_date=today(),
                completion_photo="/files/a271-twice.png",
                completion_notes="Fault cleared and retested on site",
            )
    ...
    def test_cancel_reverses_without_erasing_the_originals(self):
        ...
        with as_user(self.tech):
            ...
            mark_completed(
                wo.name,
                actual_end_date=today(),
                completion_photo="/files/a271-cancel.png",
                completion_notes="Fault cleared and retested on site",
            )
        ...
        self.assertEqual(
            frappe.db.get_value("Maintenance Request", mr.name, "status"), "Resolved",
            "completion resolves the request; closing it is a separate action",
        )
        ...
        self.assertEqual(
            frappe.db.get_value("Maintenance Request", mr.name, "status"), "Resolved",
            "a COMPLETED order that is cancelled must not reopen the request "
            "(maintenance_work_order.py:42-47) — only a Planned/In Progress one does, "
            "because the request may already be owned by newer work",
        )
