...
class TestFinancialSideEffects(ApexHabitatTestCase):
    def setUp(self):
        ...
        # [#qutlss] Pick an ACTIVE employee: the damage assessment's on_submit posts an
        # Additional Salary, which HRMS refuses for a non-Active employee, so a stray
        # Inactive pick would fail the deduction path regardless of this test's subject.
        joined_filter = ["<=", "2026-01-01"]
        self.employee = frappe.db.get_value(
            "Employee",
            {"company": self.company, "date_of_joining": joined_filter, "status": "Active"},
        )
        if not self.employee:
            self.employee = frappe.db.get_value(
                "Employee", {"date_of_joining": joined_filter, "status": "Active"}
            )
        if not self.employee:
            emp = frappe.get_doc({
                "doctype": "Employee",
    ...
    def test_damage_deduction_back_propagates_to_source_checkout(self):
        ...
        submit_via_workflow(assessment)
        assessment.reload()
