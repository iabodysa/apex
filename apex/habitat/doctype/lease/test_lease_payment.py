...
``TestRentPaymentDuplicateProbeLocks`` covers a narrower defect in the same module: the
duplicate-rent guard has to read the table it is guarding, under a lock.
...
which was taken before the lock was ever waited on — so a second concurrent click would
pass the lock, look, see nothing, and raise a second draft Payment Entry for one
...
has no write to protect. That class patches ``lease_payment.frappe`` wholesale and never
touches the database, so it runs as a plain ``unittest.TestCase``.
"""
