...
class TestStockSourceLocking(ApexHabitatTestCase):
    ...
    def _drop_committed_fixture(self):
        """Remove everything setUp created and COMMIT the removal.

        Only the concurrency case registers this. That case has to commit so a second
        connection can see the seeded rows, which puts its whole fixture past the
        FrappeTestCase rollback; a plain ``frappe.db.rollback`` would leave the
        committed rows behind. Every other case here is undone by the class rollback
        and must not be committed over.
        """
        frappe.set_user("Administrator")
        frappe.db.delete("Accommodation Stock Ledger", {"item": self.article})
        for doctype, name in (
            ("Custody Article", self.article),
            ("Building", self.source),
            ("Building", self.dest),
            ("Site", self.site.name),
        ):
    ...
    def test_concurrent_balance_lock_blocks_second_drain(self):
        ...
        self._seed_source(5)
        frappe.db.commit()
        self.addCleanup(self._drop_committed_fixture)
