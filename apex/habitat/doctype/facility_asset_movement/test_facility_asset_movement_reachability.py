...
``frappe.modules.import_file.import_doc`` sets ``frappe.flags.in_import`` (import_file.py:212)
and ``Document._set_defaults`` returns early under that flag (document.py:833-835), so the
...
class TestFacilityAssetMovementReachability(FrappeTestCase):
    ...
    def test_unauthorized_role_refused(self):
        ...
        self.assertIsNone(
            P.dual_building_scoped_has_permission(
