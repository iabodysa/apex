...
skips field defaults (`document.py:833`, under the framework's fixture-import flag), so an
omitted flag lands as 0 anyway, while an explicit `"export": 0` reads to the next editor as
a toggle someone meant to flip.
...
# Three reports named the role and threw on every click. "Active Resident Register" was
# the third; it was retired by ``apex/patches/v2_3/retire_replaced_reports.py`` and is no
# longer a Report record on any site, so only the surviving two are gradeable here.
THE_GRANTED_REPORTS = (
