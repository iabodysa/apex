...
QUERY_FN = "apex.habitat.permissions.maintenance_inspection_report_query"
...
class TestMaintenanceInspectionReportScopeRuntime(FrappeTestCase):
    ...
    def _rows_the_fragment_returns(self, user):
        ...
        """
        fragment = P.maintenance_inspection_report_query(user=user)
        if fragment == "1=0":
            return set()
    ...
    def test_a_supervisor_with_no_building_permission_sees_nothing(self):
        ...
        mine, theirs = self._pair()
        self.assertEqual(P.maintenance_inspection_report_query(user=self.unpermitted), "1=0")
    ...
    def test_the_fragment_names_the_building_column_and_only_this_estate(self):
        fragment = P.maintenance_inspection_report_query(user=self.scoped)
        self.assertIn("`building`", fragment)
        self.assertIn(self.b1, fragment)
        self.assertNotIn(self.b2, fragment)
        self.assertEqual(P.maintenance_inspection_report_query(user=self.oversight), "")
