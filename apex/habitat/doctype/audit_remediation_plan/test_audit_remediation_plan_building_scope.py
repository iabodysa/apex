...
QUERY_FN = "apex.habitat.permissions.audit_remediation_plan_query"
HANDLER = "apex.habitat.permissions.audit_remediation_plan_has_permission"
...
class TestAuditRemediationPlanScopeWiring(unittest.TestCase):
    ...
    def test_the_handler_defers_in_estate_and_denies_out_of_estate(self):
        ...
        with outer, inner:
            self.assertIsNone(
                P.audit_remediation_plan_has_permission(_plan(BLD_A), "read", user="sup"),
                "a supervisor was denied a plan naming their own building",
            )
            self.assertIs(
                P.audit_remediation_plan_has_permission(_plan(BLD_B), "read", user="sup"),
                False,
    ...
    def test_a_plan_naming_several_buildings_is_visible_if_any_one_is_mine(self):
        ...
        with outer, inner:
            self.assertIsNone(
                P.audit_remediation_plan_has_permission(
    ...
    def test_a_plan_naming_no_building_at_all_fails_closed(self):
        ...
        with outer, inner:
            self.assertIs(
                P.audit_remediation_plan_has_permission(_plan(), "read", user="sup"),
                False,
            )
            self.assertIs(
                P.audit_remediation_plan_has_permission(
    ...
    def test_oversight_defers_and_a_permissionless_scoped_user_is_denied(self):
        with patch.object(P, "_building_is_unscoped", return_value=True):
            self.assertIsNone(
                P.audit_remediation_plan_has_permission(_plan(BLD_B), "read", user="mgr")
            )
    ...
    def test_the_fragment_renders_the_two_edge_cases_the_siblings_render(self):
        with patch.object(P, "_building_is_unscoped", return_value=True):
            self.assertEqual(P.audit_remediation_plan_query(user="mgr"), "")
        outer, inner = _scoped_to([])
        with outer, inner:
            self.assertEqual(P.audit_remediation_plan_query(user="sup"), "1=0")
    ...
    def test_the_fragment_subqueries_the_child_table_and_escapes_its_values(self):
        ...
        with outer, inner, patch.object(
            ...
            fragment = P.audit_remediation_plan_query(user="sup")
...
class TestAuditRemediationPlanScopeRuntime(FrappeTestCase):
    ...
    def _rows_the_fragment_returns(self, user):
        ...
        """
        fragment = P.audit_remediation_plan_query(user=user)
        if fragment == "1=0":
            return set()
    ...
    def test_a_supervisor_with_no_building_permission_sees_nothing(self):
        mine, theirs = self._pair()
        self.assertEqual(P.audit_remediation_plan_query(user=self.unpermitted), "1=0")
    ...
    def test_the_fragment_names_only_this_estate(self):
        fragment = P.audit_remediation_plan_query(user=self.scoped)
        self.assertIn(self.b1, fragment)
        self.assertNotIn(self.b2, fragment)
        self.assertEqual(P.audit_remediation_plan_query(user=self.oversight), "")
