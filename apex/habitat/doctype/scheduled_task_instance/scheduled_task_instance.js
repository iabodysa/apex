		...
		if (frm.doc.docstatus === 1 && frm.doc.status === "Open") {
			frm.add_custom_button(__("Start Task"), function () {
				frappe.confirm(
					...
					function () {
						frappe.call({
							method: "apex.habitat.doctype.scheduled_task_instance.scheduled_task_instance.start_task",
		...
		if (frm.doc.docstatus === 1 && ["Open", "In Progress"].includes(frm.doc.status)) {
			frm.add_custom_button(__("Mark Completed"), function () {
				frappe.confirm(
					...
					function () {
						frappe.call({
							method: "apex.habitat.doctype.scheduled_task_instance.scheduled_task_instance.mark_completed",
							args: { task_instance: frm.doc.name },
							freeze: true,
							freeze_message: __("Updating status..."),
							callback: function (r) {
