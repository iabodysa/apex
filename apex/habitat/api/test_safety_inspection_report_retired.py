...
A-269 removed the workspace link and the DocType was marked ``read_only``, but
neither closed the record: root ``read_only`` is labelled "User Cannot Search"
and frappe/utils/user.py:130-144 only diverts ``can_read`` for a
read-WITHOUT-create grant, so every role holding create was untouched. Five
surfaces still reached it, and one — a whitelisted POST behind the Safety Map
page — still inserted AND submitted one on every click.
