...
The checkout clearance slip -- a sheet physically handed to a worker at
...
class TestPrintFormatRetiredNames(unittest.TestCase):
    ...
    def test_guard_actually_detects(self):
        ...
        An empty glob makes every assertion above pass vacuously — the failure mode
        that let the checkout clearance slip's retired "Accommodation Checkout
        Clearance" heading ship undetected in the first place.
        """
        ...
        # The retired heading, planted as a bare literal, must be reported.
