...
class _ScopeCase(unittest.TestCase):
    ...
    def _render(self, unscoped, buildings, filters=None):
        ...
        with patch.object(HP, "_building_is_unscoped", return_value=unscoped), \
            ...
            _columns, data = R.execute(filters or {})
...
class TestTheQueryFragment(_ScopeCase):
    ...
    def test_unscoped_renders_an_empty_string_meaning_no_restriction(self):
        with patch.object(HP, "_building_is_unscoped", return_value=True):
            self.assertEqual(HP.housing_checkout_query("mgr@example.com"), "")
    ...
    def test_scoped_with_no_buildings_renders_a_fragment_matching_nothing(self):
        with patch.object(HP, "_building_is_unscoped", return_value=False), \
            ...
            self.assertEqual(HP.housing_checkout_query("sup@example.com"), "1=0")
    ...
    def test_scoped_renders_the_bed_subquery_because_there_is_no_building_column(self):
        with patch.object(HP, "_building_is_unscoped", return_value=False), \
            ...
            self.assertEqual(
                HP.housing_checkout_query("sup@example.com"),
    ...
    def test_the_two_edge_cases_are_not_interchangeable(self):
        ...
        with patch.object(HP, "_building_is_unscoped", return_value=True):
            unscoped = HP.housing_checkout_query("mgr@example.com")
        with patch.object(HP, "_building_is_unscoped", return_value=False), \
             patch.object(HP, "_allowed_buildings", return_value=[]):
            starved = HP.housing_checkout_query("sup@example.com")
...
class TestTheHasPermissionHandler(_ScopeCase):
    ...
    def test_unscoped_defers_to_the_docperms(self):
        with patch.object(HP, "_building_is_unscoped", return_value=True):
            self.assertIsNone(
                HP.housing_checkout_has_permission(self._doc(bed="BED-B1"), "read")
            )
    ...
    def test_an_in_estate_checkout_defers_rather_than_granting(self):
        with patch.object(HP, "_building_is_unscoped", return_value=False), \
            ...
            self.assertIsNone(
                HP.housing_checkout_has_permission(self._doc(bed="BED-A1"), "read")
            )
        ...
        with patch.object(HP, "_building_is_unscoped", return_value=False), \
            ...
            for ptype in ("read", "write", "submit", "cancel", "delete"):
                with self.subTest(ptype=ptype):
                    self.assertIs(
                        HP.housing_checkout_has_permission(
    ...
    def test_the_shared_building_handler_would_have_blacked_the_role_out(self):
        """Why this DocType needed its own handler and not the sibling one.

        Housing Checkout has no `building` field, so building_scoped_has_permission reads
        None and denies — including the supervisor's OWN building. Recorded as a test so
        the handler cannot be "simplified" back to the shared one.
        """
        with patch.object(HP, "_building_is_unscoped", return_value=False), \
             patch.object(HP, "_allowed_buildings", return_value=[BLD_A]):
            in_estate = self._doc(bed="BED-A1")
            self.assertIs(
                HP.building_scoped_has_permission(in_estate, "read"), False
            )
            self.assertIsNone(HP.housing_checkout_has_permission(in_estate, "read"))
