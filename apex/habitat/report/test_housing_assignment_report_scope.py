...
"""The two Housing Assignment reports must scope through ONE seam.
...
A report can resolve it itself, calling ``permissions._building_is_unscoped`` and
``permissions._allowed_buildings`` and re-deriving the verdict inline. That is
equivalent to the seam only by coincidence of duplication: it produces the same answer
today and silently stops tracking the seam the moment the seam changes shape — the
``bed``-hop ``housing_checkout_query`` already needs is exactly such a change. Prose
cannot detect that drift, which is why the check below is a test.
...
it. Both reports reach it as an attribute of the shared ``apex.habitat.permissions``
module, so one patch reaches both. A report that resolves its scope any other way
...
class TestHousingAssignmentReportsRouteThroughTheSeam(FrappeTestCase):
    ...
    def test_an_empty_seam_returns_no_rows(self):
        ...
        querying at all."""
        for report in (R_idle, R_occupancy):
            with self.subTest(report=report.__name__):
                with patch.object(
...
class TestHousingAssignmentReportScopeIntegration(FrappeTestCase):
    ...
    def tearDownClass(cls):
        """Remove the WHOLE fixture, then commit.

        The commit is what makes the removal stick, and it also makes everything still
        pending stick — so anything left standing here is permanent on the site. The
        class therefore drops its own buildings, site, employees and project as well as
        the assignments and the User Permission, in link order.
        """
        frappe.set_user("Administrator")
        frappe.db.delete(
            "User Permission",
