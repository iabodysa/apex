...
# One dispatcher serves every building-scoped DocType; frappe passes the doctype in
# (db_query.py:1129), so the registered target is the same string for all four and the
# per-DocType difference lives in the scope table, not in the hook entry.
QUERY_TARGET = "apex.habitat.permissions.building_scope_query"
SCOPED = {
    "Safety Incident": QUERY_TARGET,
    "Safety Inspection Report": QUERY_TARGET,
    "Safety Finding Ledger": QUERY_TARGET,
    "Cleaning Compliance Ledger": QUERY_TARGET,
}
