...
class TestDocReferencePaths(unittest.TestCase):
    ...
    def test_at_least_the_known_guides_are_covered(self):
        ...
        ``docs/training/costs.md`` (cited at
        habitat/doctype/rent_payment_schedule/test_rent_payment_schedule.py:15) is the
        app's one live guide citation, so anchoring on it proves the scanner sees a real
        match instead of vacuously matching zero. Anchor on a SECOND path only when a
        second guide is genuinely cited; naming one that is not leaves this test asserting
        the presence of prose rather than the health of the scan.
        """
        cited = {reference[2] for reference in collect_references(APP_ROOT)}
        self.assertIn("docs/training/costs.md", cited)
