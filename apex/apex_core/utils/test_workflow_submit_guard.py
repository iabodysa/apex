...
Size note: 177 test lines against 37 in the subject (4.8x), for 8 distinct behaviours. The ratio is arithmetic on a small subject, not overtesting: each test names a different behaviour.
"""
...
# The two shipped drivers, one per case that needs a Driver Clearance: a clearance is the
# subject and is still built, but the person it clears is a fixture.
BLOCKED_CASE_DRIVER = "_Test Driver"
CLEARED_CASE_DRIVER = "_Test Driver Two"
...
class TestWorkflowSubmitGuard(FrappeTestCase):
    ...
    def setUpClass(cls):
        ...
        cls.project = frappe.db.get_value("Project", {"project_name": "_Test Project"})
        cls._grant_project(cls.maker, cls.project)

    @classmethod
    def tearDownClass(cls):
        # The grant is handed back; the project is NOT deleted — it is the shared fixture,
        # not this class's to dispose of.
        frappe.set_user("Administrator")
        frappe.db.delete(
            "User Permission",
    ...
    def _driver(self, fixture):
        """A shipped Salis Driver, lent this class's project for the duration of the case."""
        name = frappe.db.get_value("Salis Driver", {"full_name": fixture})
        self.addCleanup(
            frappe.db.set_value,
            "Salis Driver",
            name,
            "project",
            frappe.db.get_value("Salis Driver", name, "project"),
        )
        frappe.db.set_value("Salis Driver", name, "project", self.project)
        return name
    ...
    def test_bare_submit_driver_clearance_blocked(self):
        ...
        dc = self._driver_clearance(self._driver(BLOCKED_CASE_DRIVER))
    ...
    def test_apply_workflow_driver_clearance_submits(self):
        dc = self._driver_clearance(self._driver(CLEARED_CASE_DRIVER))
