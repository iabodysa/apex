...
class TestCancelHandlersNeverRefuse(unittest.TestCase):
    ...
    def test_baseline_holds_no_stale_entry(self):
        ...
        This entry is kept deliberately, though it grades ZERO items today: `_BASELINE` is
        empty (:75), so `stale` is always []. It is not removable, because the sibling
        above subtracts (`k not in _BASELINE`) rather than comparing sets — a stale entry
        would silently suppress a real finding there and nothing else would say so. The
        comment at :70-74 records this ratchet firing within the last cycle.
        """
