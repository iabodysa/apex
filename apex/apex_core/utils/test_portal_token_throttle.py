...
"""Portal bad-token throttle, proved in BOTH directions.
...
The counterpart integration coverage in ``test_portal_identity.py`` beside this file
exercises the same guard against real ``Masar Worker Token`` rows.

The CSRF seam is ``portal_bootstrap.get_csrf_token`` and never the www page: every entry
reaches it through ``publish_portal_context``, and no www module binds that name, so a
patch aimed at the page raises ``AttributeError`` before the throttle is ever driven.

TestThrottleAddressIsolation is the app-wide half: the window is keyed on
...
TestBadTokenAddressGuard is the address-guard half: ``test_a_console_or_job_caller_is_never_
...
from apex.apex_core.utils import portal_bootstrap, portal_identity as token_security
...
class _RestoresFrappeLocals:
    ...
    caught only by a snapshot-diff guard elsewhere, whose single baseline entry this
    file provides. ``_ABSENT`` keeps the distinction, and both classes below
...
class TestPortalTokenThrottle(_RestoresFrappeLocals, unittest.TestCase):
    ...
    def _assert_window_cleared(self, name):
        ...
        to land, not merely to have been written — a cleanup call that runs but
        leaves the key in place would otherwise pass unnoticed."""
        self.assertIsNone(
        ...
        for page, field in ((masar_page, "w"), (driver_page, "d")):
            with self.subTest(entry=page.__name__):
                ...
                with self._token_reads(), mock.patch.object(
                    portal_bootstrap, "get_csrf_token", return_value="csrf"
                ):
                    for attempt in range(LIMIT):
...
class TestThrottleAddressIsolation(unittest.TestCase):
    ...
    reason that is nowhere in its own source. Distinctness rests on this AST scan
    rather than on a 48-bit random suffix — safe by birthday odds, but uncheckable
    ...
    def test_scan_still_sees_both_portal_throttle_test_files(self):
        """The two related throttle test files must both stay IN the scan. Their subnets
        are read from source, never named here — only their presence is asserted, so
        the scan cannot go blind on exactly this pair.

        The guarded module is ``apex_core/utils/portal_identity.py``, but the test
        file beside it keeps its own name, ``test_portal_token_security.py`` -- a
        file rename is a separate change from a module re-aim.
        """
