...
"""The shared atomic fixed-window counter, proved against the shape it replaced.
...
Size note: 114 test lines against 23 in the subject (5.0x), for 6 distinct behaviours. The ratio is arithmetic on a small subject, not overtesting: each test names a different behaviour.
"""
...
class TestChargeWindow(unittest.TestCase):
    ...
    # `test_concurrent_callers_each_receive_a_distinct_count` is not defined here: it would
    # drive `cache.eval` directly across 16 threads and assert `sorted(seen) == [1..16]` —
    # only what INCR always guarantees, so it would pass against any INCR-based script that
    # never calls `charge_window`, and it owned the file's only ThreadPoolExecutor.
    # Distinctness is proved structurally by
    # `test_the_charge_never_reads_the_counter_before_writing_it` (one EVAL, zero GET/SETEX)
    # and behaviourally by `test_a_stale_first_request_cannot_reset_a_shared_window`.
