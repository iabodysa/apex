...
"""What the four guest SUBMIT endpoints and the Front Desk scan read COUNT, proven by spending
their real windows.

SIZE, DELIBERATE: this file exceeds the 2x rule against ``rate_limit_identity.py`` (63 lines)
and stays -- that decorator's CONTRACT covers 44 whitelisted endpoints across habitat and salis,
four unauthenticated guest WRITES, and the four insert caps in
``arrival_batch``/``arrival_manifest``. It spends each window call by call against the
framework's own limiter, and holds the two population ratchets (RATE_LIMITED_ENDPOINTS,
DRIVER_PORTAL_RE_EXPORTS) that are the only signal an endpoint quietly lost its decorator --
measured against that surface, not the decorator's own line count, it is not oversized.

``@rate_limit``'s ``key`` is not the request attribute it reads like: a form_dict lookup --
``frappe.form_dict.get(key, "")`` (rate_limiter.py:143) -- over a dict built from the query
string and the body (app.py:302-314), feeding an identity of ``<ip>:<user_key>``
(rate_limiter.py:145-150). So ``key="frappe.request.remote_addr"`` never reads an address: it
reads a field no honest caller sends, yields "", and names a bucket ``rl:<cmd>:<ip>:`` -- a
caller who DOES send that field buys a private window per value. 38 guest driver endpoints
(salis/api/test_rate_limit_identity.py) close the same defect elsewhere; these five are the
worst of what is left -- unauthenticated ``allow_guest`` POST endpoints that INSERT a document
(Resident Request, Arrival Batch, Transport Request, Vehicle Incident), where a forged window
buys unbounded WRITES from any address, with only the honeypot in the way. They now pass no
``key``: ``ip_based`` defaults to True (rate_limiter.py:110) and already puts the address in the
identity (rate_limiter.py:141,147-150), so the ceiling is unchanged and the caller-chosen suffix
is gone.

The window's NAME was the same bug through another door: frappe names it after the caller's
``cmd`` (rate_limiter.py:155) but resolves the handler separately, so two paths to one function
were two ceilings -- 35 over 61 endpoints. Apex names every window after the HANDLER
(rate_limit_identity.py); this file spans the full 61. The ceiling here is not derived from the
decorator's arguments, which would only restate the edit -- the decorator is parsed for ONE
thing, where each ceiling is DECLARED, while what is asserted is the ceiling frappe ENFORCES,
spent call by call, with the attack replayed WHILE it is spent: every call rotates the query
parameter, so a still-partitioned window would open fresh on every call and no refusal could
arrive.

Hermetic on purpose -- a throttle whose guards only run against a live bench is a throttle whose
guards do not get tested: the counter is a fake in-process cache, the address is stubbed, and
the limiter, the identity, the window length and the exception are frappe's own. The five
endpoints straddle habitat and salis, so neither module's tree is an honest owner and
tests/test_colocation_ratchet.py has the central directory closed; it sits in the shared kernel
beside the other cross-module throttle guards it reasons about -- rate_window.py,
request_ip_trust.py and test_portal_token_throttle.py, the app-wide address-isolation guard this
file's subnet answers to.
"""
...
# `driver_portal.fuel` does not exist -- the package holds only execution, images,
# personal, profile and trips -- so this anchors on the surviving pair of identical
# shape: `start_my_trip`, re-exported by the package (__init__.py:269) and defined in
# execution.py:46 under @rate_limit(limit=10).
...
# This count is 44 -- 17 lower than the endpoint list would otherwise suggest, because
# the driver portal's fuel/support/attendance/clearance/boarding/notifications modules
# do not exist; that gap is not a regression.
RATE_LIMITED_ENDPOINTS = 44
...
# This count is 9; the same missing modules noted above account for the gap from a
# theoretical 32.
DRIVER_PORTAL_RE_EXPORTS = 9
...
class TestSubmitRateLimitIdentity(unittest.TestCase):
    ...
    # `test_a_query_parameter_no_longer_buys_a_private_window` is not defined here: it is
    # strictly contained in the case below, which rotates the parameter on EVERY call
    # rather than three times. Its distinctive assertions — no `rl:<cmd>:<ip>:<value>`
    # bucket, no bare-colon bucket — follow from `assertEqual(cache.charged(), {shared})`,
    # because `_FakeCache.charged()` (:378) returns every key ever written, not the
    # matching ones.
    ...
    def test_the_two_live_spellings_of_a_driver_endpoint_share_one_window(self):
        ...
        Not a synthetic second path: ``apex.salis.api.driver_portal.start_my_trip`` is
        the package spelling the SPA sends and
        ``...driver_portal.execution.start_my_trip`` is where the function is defined
        (execution.py:46), so both resolve and both are live. Under the framework's own
        naming this admits 2x the ceiling; the control below still measures exactly that,
        so a pass here cannot be the fake cache being generous.
        ...
        """
        self.assertIs(driver_portal.start_my_trip, execution.start_my_trip)
        endpoint = driver_portal.start_my_trip
        limit = _declared_limits(execution, "start_my_trip")["limit"]
        spellings = (
            "apex.salis.api.driver_portal.start_my_trip",
            "apex.salis.api.driver_portal.execution.start_my_trip",
        )
        admitted = 0
        cache = _FakeCache()
        with mock.patch.object(frappe, "cache", cache):
        ...
        self.assertEqual(
            admitted,
            limit,
            f"start_my_trip admitted {admitted} against its ceiling of {limit}: "
            "the second dotted path still buys a window of its own",
        )
        self.assertEqual(
            cache.charged(),
            {_canonical_window(execution, "start_my_trip", _STUB_IP)},
    ...
    def test_every_second_dotted_path_reaches_an_identity_keyed_endpoint(self):
        ...
        being the shape that actually exists here.
        ...
        The scan's own self-check is anchored on the live production re-export the case
        above already drives by hand -- a name inside the population being graded, so a
        scan that goes quiet still cannot read as a clean package. It cannot anchor on
        this file's own module-level import instead: no test module ships inside the
        package, so an import here would name a file the scan can never reach, and the
        case would die before grading anything.
        """
        endpoints = _rate_limited_endpoints()
        ...
        self.assertIn(
            "apex.salis.api.driver_portal.start_my_trip",
            aliases,
            "the scan did not record the driver portal re-export this file drives by "
            "hand, so an empty result below would mean a broken scan rather than a "
            "clean package",
        )
        self.assertGreater(
            len(aliases),
            500,
            f"the alias scan reached only {len(aliases)} module-level names — it is "
            "reading a tree that is not the installed package",
        )
        self.assertEqual(
            _relative_imports(),
            [],
        ...
        # An `unguarded` block, listing second paths whose target was not keyed on
        # IDENTITY_MODULE, is not defined here: it is strictly implied by the case above,
        # which asserts EVERY endpoint's rate_limit source is IDENTITY_MODULE — so no
        # alias can point at one that is not, whatever the alias set contains.
