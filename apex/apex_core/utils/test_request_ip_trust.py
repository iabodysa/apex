...
"""The XFF trust boundary, proved against frappe's REAL resolver.
...
unset-versus-None distinction: both names are written before the
...
class TestNothingUnproveableIsCertified(unittest.TestCase):
    ...
    def test_an_entry_that_does_not_parse_is_refused_rather_than_ignored(self):
        ...
        treating every rendering the parser cannot read as absence would make the
        set of strings that silently pass a deployment infinite and growing with each
        form some future edge invents. Read that way, each of these four would return
        "overwritten" with trusted true while sitting unchanged in the header, which is
        a header the edge did NOT overwrite. Refusing what cannot be read instead turns
        the spelling table into a convenience instead of the security boundary."""
        for entry in (
            f"{PROBE_ADDRESS}/32",
