...
class TestRetentionSetting(FrappeTestCase):
    ...
    def test_stored_value_wins(self):
        ...
        self.assertEqual(retention_days(RETENTION_KEY), 120)

    def test_effective_uses_setting_when_none_passed(self):
        _set_single("Apex Settings", RETENTION_KEY, 200)
        self.assertEqual(effective_retention_days(RETENTION_KEY, None), 200)
    ...
    def test_effective_setting_overrides_untouched_hook_seed(self):
        ...
        self.assertEqual(
            effective_retention_days(RETENTION_KEY, RETENTION_DEFAULTS[RETENTION_KEY]), 45
        )

    def test_effective_honours_explicit_operator_override(self):
        _set_single("Apex Settings", RETENTION_KEY, 45)
        self.assertEqual(effective_retention_days(RETENTION_KEY, 17), 17)
