...
Fixtures are minted per METHOD. FrappeTestCase rolls back rows once per CLASS, so
a driver or user shared between methods carries the previous method's mutations.
"""
...
TOKEN_JSON = Path(__file__).resolve().parent / "masar_worker_token.json"
...
class _DeskIssuanceCase(FrappeTestCase):
    """Fixtures for a desk-issuance case, minted fresh for every method."""
    ...
    def _tag(self):
        return frappe.generate_hash(length=12).lower()

    def _project(self):
        return make_project(f"A267-{self._tag().upper()}")

    def _driver(self, project=None, status="Active"):
        """A bare Salis Driver: no User, no Employee.

        The shared driver factories build a User+Employee+Driver chain, which is
        the opposite of the subject here — the whole point of the barcode is a
        driver with no account at all."""
        return (
            frappe.get_doc(
                {
                    "doctype": "Salis Driver",
                    "full_name": f"A267 Driver {self._tag()}",
                    "status": status,
                    "project": project,
                }
            )
    ...
    def _issuer(self, role, project=None):
        user = _user(f"a267-{self._tag()}@example.com", role)
        if project:
            _grant_project(user, project)
        return user
...
class TestDriverLinkDeskRevocation(_DeskIssuanceCase):
    def test_out_of_scope_supervisor_cannot_revoke_while_the_owning_one_can(self):
        ...
        mine = self._project()
        theirs = self._project()
        outsider = self._issuer("Fleet Supervisor", project=mine)
        owner = self._issuer("Fleet Supervisor", project=theirs)
