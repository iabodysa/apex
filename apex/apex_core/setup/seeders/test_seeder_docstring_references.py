...
The shapes this catches, all seen in this one package: a seeder naming a sibling
module archived with an engine, a seeder naming a run-once patch pruned as spent,
and a seeder claiming to mirror a file retired for native JSON. Each sends the next
reader hunting something deliberately removed, and nothing on the tree says
otherwise -- prose is the one place a rename does not break a build.
...
SEEDERS_DIR = os.path.join(str(Path(apex.__file__).resolve().parent), "apex_core", "setup", "seeders")
...
class TestSeederDocstringReferences(unittest.TestCase):
    ...
    def test_a_live_reference_still_resolves(self):
        ...
        ``salis_issue_seed`` and ``patches/v0_8/add_navbar_help_links.py`` no
        longer name anything in the tree -- the issue-seeding call moved into
        ``apex_core/setup/salis_support.py`` and the ``patches/v0_8`` directory is
        gone -- so this now names a seeder module and a sibling-seeder path that
        currently ship, the same two shapes the retired tokens covered.
        """
        for token in ("module_profile_seed", "habitat_auto_email_reports_seed.py"):
