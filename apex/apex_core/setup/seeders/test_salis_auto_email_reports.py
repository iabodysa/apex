...
Size note: 54 test lines against 10 in the subject (5.4x), for 4 distinct behaviours. The ratio is arithmetic on a small subject, not overtesting — each test names a different one.
"""
...
DAILY_REPORT = "Vehicle Assignment Register"
...
class TestSalisAutoEmailReportSeeder(FrappeTestCase):
    def setUp(self):
        ...
        # mid-test failure still restores.
    ...
    def test_daily_digest_report_renders_columns(self):
        columns = frappe.get_attr(
            "apex.salis.report.vehicle_assignment_register.vehicle_assignment_register.execute"
        )(None)[0]
        fieldnames = {c["fieldname"] for c in columns}
        self.assertIn("vehicle", fieldnames)
        self.assertIn("held", fieldnames)
