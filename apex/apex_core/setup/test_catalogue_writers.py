...
"""Cross-writer guards for Maintenance Material, the one catalogue left with TWO
install-time writers.

Maintenance Material is written by a module list (38 records) AND by a seed data
JSON (60 records). Nothing checked the halves against each other, which is how the
default Maintenance Material Templates came to carry 13 child rows pointing at
materials only the JSON half supplied.

Safety Task Catalog held a second writer here too: a ``create_safety_task_catalogs``
function in ``apex/setup.py``. That function is retired, and the catalogue's seed
data JSON (``apex_core/setup/data/habitat/safety_task_catalog.json``, 60 records) is
its one remaining writer, so the cross-writer guards below cover Maintenance
Material only.

Two ways two writers of one catalogue can disagree, one test each:
...
2. their union no longer covers a key some other seeded record links to — the
   template's child Link dangles and Frappe refuses the whole template.

Pure: reads the JSON off disk and the writer's module-level constant by AST, so no
site and no ``frappe`` import is needed.
"""
...
# named `.claude/tests`, where none of the sources read below exist.
_REPO_ROOT = str(Path(apex.__file__).resolve().parents[1])
