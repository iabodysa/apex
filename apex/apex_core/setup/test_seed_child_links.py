...
``apply_spec`` inspects Link fields at BOTH the top level and inside child tables: a
record whose CHILD rows point at missing targets must fail this guard rather than
reach ``doc.insert()``, which raises ``LinkValidationError`` there instead because
Frappe's own ``_validate_links`` DOES walk children. Missing that check would leave
records like the default Maintenance Material Templates unbuildable on a fresh install.
...
# named `.claude/tests`, where none of the DocType JSON read below exists.
