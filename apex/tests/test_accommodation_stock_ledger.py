...
class TestAccommodationStockLedger(ApexHabitatTestCase):
    ...
    def test_reverse_negates_and_cancels(self):
        ...
        self.assertEqual(flt(sum(flt(r.signed_qty) for r in rows)), 0.0, "net balance after reversal must be 0")
    ...
    def test_signed_qty_accepts_negative_value(self):
        """A reversal/drain entry must carry a negative signed_qty; the field and
        contract name must both reflect the signed convention."""
        name = post_stock_entry(
            item_type="Custody Article", item=self.article.name, qty=-3,
            building=self.building, employee=self.employee,
            voucher_type="Test Voucher", voucher_no="TV-NEG-1", voucher_detail_no="r1",
        )
        doc = frappe.get_doc("Accommodation Stock Ledger", name)
        self.assertLess(flt(doc.signed_qty), 0, "signed_qty must be negative for a drain/reversal entry")
