...
# Anti-rubber-stamp floors. A reason short enough to fail these is a
...
def _registered_patches():
    ...
    for section in parser.sections():
        entries.extend(_dotted_entry(entry) for entry in parser[section])
    return parser.sections(), entries


def _dotted_entry(entry):
    """The module an entry names, with its trailing date tag dropped.

    ``execute_patch`` takes ``patchmodule.split(maxsplit=1)[0]``
    (frappe/modules/patch_handler.py:167), so everything after the first space is a
    comment frappe never resolves. Reading the whole line as the dotted path made every
    dated entry look like a patch with no module behind it.
    """
