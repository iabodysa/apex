...
class TestBreadcrumbRootPin(unittest.TestCase):
    def setUp(self):
        ...
        # name -> module, flattened once, so a landing check can tell whether a
        # workspace's parent is inside or outside its own module.
        self.module_by_name = {
            row["name"]: module for module, rows in self.by_module.items() for row in rows
        }
    ...
    def _is_module_root(self, row, module):
        """True when nothing WITHIN THE SAME MODULE sits above this workspace.

        Under the shipped shape only the global ``Apex`` workspace is parent-less;
        every other module (Habitat, Salis, Logistay) hangs its own root off ``Apex``
        (see CLAUDE.md: "Apex Core, My Tasks, Habitat, Salis, Logistay hang off
        Apex"). A module's breadcrumb target is therefore not required to be
        globally parent-less - it only needs no ancestor inside its own module, i.e.
        it is the top of that module's own subtree. A workspace whose parent belongs
        to a DIFFERENT module (or has no parent at all) qualifies.
        """
        if not row["parent"]:
            return True
        return self.module_by_name.get(row["parent"]) != module
    ...
    def test_every_module_declares_one_visible_landing_root(self):
        ...
        "Landing root" means the top of the MODULE's own subtree (see
        ``_is_module_root``), not a globally parent-less workspace - the shipped
        shape nests every module but Apex Core under the ``Apex`` root, so a
        global-parent-less requirement would fail every one of them.
        """
        for module, rows in sorted(self.by_module.items()):
            candidates = self._crumb_candidates(rows)
            if not candidates:
                continue
            landing = [r for r in candidates if self._is_module_root(r, module) and not r["hidden"]]
            self.assertEqual(
                len(landing),
                1,
                f"module {module!r} must ship exactly one public, visible workspace "
                f"that is the top of its own module's subtree to be its breadcrumb "
                f"target; found {[r['path'] for r in landing]}",
            )
