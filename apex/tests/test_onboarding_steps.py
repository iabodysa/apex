...
# workspace that embeds each onboarding and gates the role.
DAILY_FLOW_CONTRACT = (
    (
        "Resident Supervisor",
        "Housing",
...
class TestOnboardingSteps(FrappeTestCase):
    ...
    def test_each_daily_role_has_at_least_two_native_tours(self):
        ...
        tours = _source_records("Form Tour", "form_tour")
        role_modules = {
            "Resident Supervisor": {"Accommodation Go-Live", "Custody Go-Live"},
            "Fleet Supervisor": {"Salis Fuel Setup"},
            "Safety Officer": {"Safety Readiness"},
            "Maintenance Technician": {"Maintenance Daily Flow"},
        }
        bad = []
